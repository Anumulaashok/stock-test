...
## 2026-09-05 — Wave 6: position-size calculator is the one place I2 is deliberately not applied, and two bugs actual tests caught

**Finding:** `PortfolioPage` needed a top contributors/detractors panel and a position-size tool. The former is pure sort/select over the backend's own `unrealized_gain_percent` (safe under I2). The latter genuinely computes a new number (share count, position value, risk amount) from user-typed inputs and a real quote -- normally exactly the class of thing I2 reserves for the backend.

**Chosen:** built the position-size calculator client-side anyway, because I3 explicitly carves this out by name ("a position-size calculator is permitted -- arithmetic over user input and a real quote"). Applied the rest of I3/I11 strictly regardless: permanent `Scenario` badge, a disclaimer that this is not a trade ticket, and no buy/sell/execute affordance anywhere (a dedicated test asserts this). `calculatePositionSize` returns `null` (not a fabricated 0/Infinity) when entry price equals stop price, since per-share risk is genuinely undefined there, not zero.

**Two real bugs a written-first test caught, not assumed correct:** (1) the calculator originally called the shared `formatCurrency()` on the raw `current_price` string after a quote lookup, producing `"₹2,500"` instead of the backend's own `formatted_current_price: '₹2,500.00'` -- fixed to use the backend's formatted string directly, consistent with the session-wide rule to never reformat a raw value the backend already formatted. (2) a macOS case-insensitive-filesystem collision between `contributorsDetractors.ts` and `ContributorsDetractors.tsx` (same name, differing only in the first letter's case) resolved to the same module under TypeScript, surfacing first as a confusing runtime React error in `vitest` rather than a naming error -- fixed by renaming the pure-logic module to `topContributorsDetractors.ts`.

**Why this matters beyond the two bugs:** neither would have been caught by a green test suite written to already-buggy expectations -- both surfaced because a test was written against the *correct* expected behavior first (the formatted-string case) or because `tsc -b` was actually run rather than trusting isolated `vitest` runs (the casing case, where `vitest` alone gave a misleading downstream error).

---

## 2026-09-05 — Wave 8: a CSS comment's own text broke the light theme it was documenting

**Finding:** the light-theme override block (`:root[data-theme='light']`) had zero visible effect on first screenshot despite `data-theme="light"` being correctly set on `<html>` -- not a subtle styling miss, a total no-op.

**Root cause, confirmed rather than guessed:** the doc comment directly above the rule contained the substring `--color-*/--chart-*` as prose ("every shared primitive already reads exclusively from --color-*/--chart-*"). CSS has no escape for `*/` inside a comment -- that substring closed the comment early, turned the rest of the comment's own text into parsed (garbage) CSS, and left a stray `*/` that corrupted the following selector into invalid `: root[data-theme='light']` once run through the dev server's CSS minifier. Confirmed by iterating `document.styleSheets` in a live page: the light-theme rule was entirely absent from the parsed stylesheet, not merely overridden.

**Fix:** reworded the comment to avoid an embedded `*/`-shaped substring ("--color- and --chart- tokens" instead of "--color-*/--chart-*"). Verified by re-fetching the dev server's served CSS and confirming the selector round-tripped clean, then re-screenshotting three real pages (Discover, a stock Overview, a stock Forecast chart) in light mode.

**Why this matters beyond the one bug:** a `/* ... */` comment mentioning two hyphenated-prefix token families side by side (`--foo-*` and `--bar-*`) is a real, easy-to-write CSS trap -- the fix generalizes to "never write a bare `*/`-shaped substring inside a CSS comment," not just this one instance. Caught by actually screenshotting the change rather than assuming a `:root[data-theme]` override block works because it type-checks/lints clean -- CSS has no compiler to catch this class of error, only visual verification does.

**Separately, this wave's chart-color refactor found the same "assumed but never checked" gap as Wave 3's AutoReg bug:** the master brief's own D8 stated "Chart.js reads CSS variables into canvas at construction" -- `grep`ping `PriceChart.tsx`/`AccuracyScatterChart.tsx` showed neither ever read a single CSS variable; every series/grid/tick/border color was a hardcoded literal, including one (`#eef1fb`) that was literally the dark theme's own `--color-text` value baked in as a point border, and another (`rgba(255,255,255,0.5)`) that would have been an invisible white halo on a light background. Investigated the actual code before trusting the brief's framing, same as Wave "ML forecasting" and the Wave 7 Fundamentals-responsive item earlier this session -- three separate instances now of a brief's stated-as-fact premise not matching the real implementation.
