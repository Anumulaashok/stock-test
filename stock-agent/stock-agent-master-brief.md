...
- **Price chart on Technical and Overview**, line only, DMA50/DMA200 overlays with toggle chips. The Screener import already returns DMA50/DMA200/volume alongside price — overlays are a fetch, not a computation. **First check whether `report.forecast.historical_prices` (already passed into `MlForecastPanel`) is the same series**; if so, no new fetch (G5).
